...
data class DeviceInfo(
    ...
    val serial: String = try { Build.getSerial() } catch (_: Exception) { Build.SERIAL ?: "unknown" },
    ...
    val isCharging: Boolean get() = batteryLevel >= 0
}
...
object DeviceInfoProvider {
    fun get(context: Context): DeviceInfo {
        ...
        return DeviceInfo(
            resolution = res,
            batteryLevel = if (level in 0..100) level else -1
        )
    }
}
